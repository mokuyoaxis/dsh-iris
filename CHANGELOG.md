...
- 增加正式视觉资产集：像素块几何母题的 IRIS 标志与图形标（彩色/单色/反白共 6 个 SVG），由 `scripts/gen-logo.mjs` 从设计矩阵可复现生成，含最小尺寸、留白与配色规范（`docs/assets/logo/`）；悬浮泡泡改用同源内联像素图形标，替换 🫧 表情；0.1.x 渐变临时字标不再使用，源码文件暂留作历史兼容。
...
- 阿里云百炼支持独立 Workspace `mediaBaseUrl`、官方模型目录分页与能力元数据；模型池保持有序安全 failover，并保护用户手工能力覆盖。
...
- 同一物理数据根只允许一个 writer；reader 不创建、修复或迁移数据。
- Headless Command 结果和错误不返回输入文件或数据根的绝对路径。
