...
/** 严格选择：只挑声明/推断出该能力的 provider；无则 null（不再兜底到任意 provider） */
export function pickFor(capability) {
  return cap.pickFor(providers(), capability);
}

/** 同一能力的有序 provider 列表（按 providers.json 顺序，供 failover 依次尝试） */
export function pickAllFor(capability) {
  return cap.providersWith(providers(), capability);
}

/** 某 provider 的实际能力列表（含旧配置迁移结果） */
export function capabilitiesOf(p) {
  return cap.capabilitiesOf(p);
}
