...
import { selectMediaProtocol, isConfiguredProvider } from './provider-protocol.js';
...
function validateStoredConfig(value) {
  ...
  for (const provider of value.providers) {
    ...
    if (provider.mediaBaseUrl !== undefined && typeof provider.mediaBaseUrl !== 'string') throw new Error('provider.mediaBaseUrl 必须是字符串');
...
/**
 * 供应商认证方式：'bearer'（默认，需要 apiKey）或 'none'（本地/内网无鉴权端点）。
 * T-01：解除 ComfyUI、Ollama、本地 TTS 等本地部署的硬编码阻塞（审计 C-1）。
 * 无 auth 字段时一律视为 'bearer'——现有用户行为完全不变。
 */
function authModeOf(provider) {
  return provider && provider.auth === 'none' ? 'none' : 'bearer';
}

/** 该供应商是否缺少必需凭据（'none' 认证永不缺）。 */
function missingCredentials(provider) {
  if (!provider || authModeOf(provider) === 'none') return false;
  return typeof provider.apiKey !== 'string' || !provider.apiKey.trim();
}

function normalizeStoredProvider(provider) {
  const selected = selectMediaProtocol(provider);
  const changed = provider.mediaProtocol !== selected.mediaProtocol
    || (provider.protocolInferred === true) !== selected.protocolInferred;
  Object.assign(provider, selected);
  return models.migrateProviderModels(provider) || changed;
}
...
export function importFromWorkbench(workbenchConfigPath) {
  ...
  for (const p of list) {
    ...
    const imported = {
      ...
      name: p.name || p.id,
      type: typeof p.type === 'string' && p.type.trim() ? p.type.trim() : 'openai',
      baseUrl: p.baseUrl,
      apiKey: p.apiKey,
      enabled: p.enabled !== false,
      mediaProtocol: p.mediaProtocol
    };
    normalizeStoredProvider(imported);
    c.providers.push(imported);
    n++;
  }
  if (n) persist();
...
export function providers() {
  return load().providers.filter(isConfiguredProvider);
}
...
export function pickAllFor(capability) {
  const pool = models.modelPool(providers());
  const chosen = models.orderedModels(pool, capability, assignments()[capability]);
  const out = [];
...
const resolveAssignment = models.resolvePoolModel;
...
const HEALTH_SENSITIVE_FIELDS = new Set([
  'auth', 'type', 'baseUrl', 'mediaBaseUrl', 'apiKey', 'mediaProtocol',
...
export function modelHealth(id, modelId, capability, { now = Date.now() } = {}) {
  ...
  if (!provider || provider.enabled === false || missingCredentials(provider)) return { status: 'unconfigured' };
...
export function upsert(provider) {
  ...
  if (id) {
    ...
    if (i >= 0) {
      ...
      if (Object.prototype.hasOwnProperty.call(rest, 'mediaProtocol')) next.protocolInferred = false;
  ...
  if (Array.isArray(modelEntries)) {
    ...
    p.models = modelEntries.map((m) => {
      ...
      if (old) {
        ...
        if (old.source === 'manual' && Array.isArray(old.capabilities)) entry.capabilities = old.capabilities;
      }
...
/** 发现只追加或更新；用户手工项和未再返回的旧项始终保留。 */
export function mergeDiscoveredModels(id, modelEntries) {
  const provider = load().providers.find((p) => p.id === id);
  if (!provider) return null;
  const merged = new Map(models.providerModels(provider).map((m) => [m.id, m]));
  for (const model of modelEntries) {
    const previous = merged.get(model.id);
    if (previous?.source === 'manual') continue;
    merged.set(model.id, { ...previous, ...model, source: 'discovered' });
  }
  return setProviderModels(id, [...merged.values()]);
}
