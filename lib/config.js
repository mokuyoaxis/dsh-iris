...
export function pickFor(capability) {
  ...
  let m = null;
  for (const id of assignmentOrder(capability)) {
    m = pool.find((x) => x.id === id && x.capabilities.includes(capability));
    if (m) break;
  }
...
export function pickAllFor(capability) {
  ...
  for (const id of assignmentOrder(capability)) {
    for (const m of pool) if (m.id === id && m.capabilities.includes(capability)) add(m);
  }
...
/** 能力 → 模型 的分配映射（用户手动指定；值可为单模型 id 或有序 failover 列表） */
export function assignments() {
  return load().assignments || {};
}

/** 归一化某能力的有序分配列表（向后兼容旧的单字符串写法）→ [modelId, ...] */
export function assignmentOrder(capability) {
  const a = assignments()[capability];
  if (!a) return [];
  return (Array.isArray(a) ? a : [a]).filter((x) => typeof x === 'string' && x);
}
 ...
 */
export function setAssignmentOrder(capability, modelIds) {
  const c = load();
  if (!c.assignments) c.assignments = {};
  ...
  for (const raw of modelIds) {
    ...
    if (!pool.some((m) => m.id === id && m.capabilities.includes(capability))) return false;
    ids.push(id);
  }
  if (!ids.length) return false;
  c.assignments[capability] = ids;
  persist();
  return true;
}
