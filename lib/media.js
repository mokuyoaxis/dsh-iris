...
export function serveMedia(req, res) {
  try {
    ...
    // Task media: /iris/media/:taskId/:token/:name
    // Independent work: /iris/media/artifact/:artifactId/:token/:name
    const artifactRoute = parts.length === 6 && parts[0] === 'iris' && parts[1] === 'media' && parts[2] === 'artifact';
    const taskRoute = parts.length === 5 && parts[0] === 'iris' && parts[1] === 'media';
    if (!artifactRoute && !taskRoute) return sendText(res, 404, 'not found');
    let name;
    try {
      name = decodeURIComponent(parts[artifactRoute ? 5 : 4]);
    } catch (_) {
      return sendText(res, 404, 'not found');
    }
    const hit = artifactRoute
      ? artifacts.authorize(parts[3], parts[4], name)
      : authorizeMedia(parts[2], parts[3], name);
