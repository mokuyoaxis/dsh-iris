  ...
  function apply(ctx) {
    ...
    function artifactPreview(item) {
      ...
      if (mime.indexOf('image/') === 0) {
        return React.createElement('a', { className: 'iris-gallery-preview', href: item.url, target: '_blank', rel: 'noreferrer', title: '打开原图' },
          React.createElement('img', { src: item.url, alt: 'Iris 作品 ' + item.file, loading: 'lazy' }));
      }
    ...
    function ArtifactGallery(props) {
      ...
      function load(append) {
        ...
        fetch('/iris/api/artifacts?offset=' + offset + '&limit=24')
          .then(function (res) { return res.ok ? res.json() : Promise.reject(new Error('读取失败')); })
          .then(function (data) {
      ...
      React.useEffect(function () { load(false); }, [props.total]);

      return React.createElement('div', {},
        React.createElement('div', { className: 'iris-gallery-tools' },
          React.createElement('span', { className: 'iris-clean-note' }, '共 ' + total + ' 个作品 · 清任务历史不会删除'),
          React.createElement('button', { className: 'iris-clean-btn', disabled: busy, onClick: function () { run('artifacts_reindex'); } }, '找回本地作品'),
          total ? React.createElement('button', { className: 'iris-clean-btn danger', disabled: busy, onClick: function () { run('artifacts_clear', {}, '永久删除作品库中的全部媒体文件？任务历史会保留，但相关媒体将无法打开。此操作不可逆。'); } }, '清空作品库') : null),
        items.length ? React.createElement('div', { className: 'iris-gallery-grid' }, items.map(function (item) {
          return React.createElement('div', { key: item.id, className: 'iris-gallery-card' },
            ...
            React.createElement('div', { className: 'iris-gallery-info' },
              ...
              React.createElement('div', { className: 'iris-gallery-meta' }, fmtTime(item.createdAt) + ' · ' + fmtBytes(item.size)),
        ...
        })) : React.createElement('div', { className: 'iris-wb-empty' }, busy ? '正在读取作品…' : '尚无作品；如 outputs/ 中已有文件，可点“找回本地作品”。'),
        items.length < total ? React.createElement('button', { className: 'iris-clean-btn', disabled: busy, onClick: function () { load(true); } }, busy ? '加载中…' : '加载更多') : null,
    ...
    function BubblePanel() {
      ...
      var works = (state && state.artifacts && state.artifacts.recent) || [];
      var artifactTotal = (state && state.artifacts && state.artifacts.total) || works.length;
