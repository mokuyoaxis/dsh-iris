...
window.__ModuleLoader__.load({ id: '@mokuyoaxis/dsh-iris', factory: (require) => {
  ...
  var cssText = [
    ...
    '.iris-bubble-panel { position: fixed; z-index: 9998; width: 360px; max-height: min(520px, 70vh); overflow: auto; background: var(--dsw-alias-bg-layer-1, #26262e); border: 1px solid var(--dsw-alias-border-l1, #3a3a44); border-radius: 12px; padding: 12px 14px; font-size: 12px; line-height: 1.45; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.45); }',
  ...
  function apply(ctx) {
    ...
    /** 功能卡就绪态直接来自共享 Iris 状态，不再等用户点开卡片后才加载。 */
    function capabilityReady(state, capability) {
      if (!capability) return true;
      var providers = (state && state.providers) || [];
      return providers.some(function (p) {
        return p.enabled && p.apiKeyHint && Array.isArray(p.capabilities) && p.capabilities.indexOf(capability) >= 0;
      });
    }
    ...
    function TaskRecoveryActions({ detail }) {
      ...
      if (!canAcknowledge && !canRestore && !canReobserve && !canRedeliver && !canRetry) return null;
    ...
    function TaskDetailDrawer({ taskId }) {
      ...
      if (Number(detail.schemaVersion) === 1) {
        el.push(React.createElement('div', { key: 'legacy', className: 'iris-clean-note' },
          '旧版任务记录 · 仅支持查看。由于缺少可靠的远端受理与交付事实，不提供重新观察、重新交付或知情重试；0.1.3 新任务将使用可信任务语义。'));
      }
      ...
      el.push(React.createElement(TaskRecoveryActions, { key: 'recovery', detail: detail }));
    ...
    function ModelPool(props) {
      ...
      function verBadge(m, c) {
        ...
        var cls = 'iris-pm-ver' + (v ? (' ' + (v.ok ? 'ok' : 'bad')) : '');
        var title = v ? ((v.ok ? '实测通过：' : '实测未通过：') + (v.note || '')) : '点击单独验证此能力';
        return React.createElement('button', { key: c, className: cls, title: title, onClick: function () { props.testModel(p, m, c); } }, c + (v ? (v.ok ? '✓' : '✗') : '?'));
      }
    ...
    function ProviderManager() {
      ...
      var newForm = React.useState({ name: '', baseUrl: '', apiKey: '', type: 'openai', mediaProtocol: 'auto' });
      ...
      var cards = (list || []).map(function (p) {
        ...
        if (open) {
          body = React.createElement('div', { className: 'iris-pm-body' },
            ...
            React.createElement('div', { className: 'iris-pm-field' }, React.createElement('label', {}, '媒体协议'), React.createElement('select', { value: p.mediaProtocol || 'openai-images', onChange: function (e) { setProtocol(p, e.target.value); } },
      ...
      if (adding) {
        addBlock = React.createElement('div', { className: 'iris-pm-card' },
          ...
          React.createElement('div', { className: 'iris-pm-field' }, React.createElement('label', {}, '媒体协议'), React.createElement('select', { value: newForm[0].mediaProtocol, onChange: function (e) { newForm[1]({ ...newForm[0], mediaProtocol: e.target.value }); } },
            React.createElement('option', { value: 'auto' }, '自动（按 Base URL 安全判断）'),
    ...
    function ActionCard(props) {
      ...
      var headError = !!(run && run !== 'running' && run.ok === false);
      var headLit = !capability || (capabilityReady(irisState, capability) && !headError);
      var headTitle = headError ? '最近一次执行出错' : (headLit ? 'API 与能力已配置' : '缺少可用 API 或能力模型');
      return React.createElement('div', { className: 'iris-act-card' + (open ? ' open' : '') + (headLit ? ' lit' : ' dim') },
        React.createElement('div', { className: 'iris-act-head', onClick: function () { setOpen(!open); } },
          ...
          capability ? React.createElement('span', { className: headLit ? 'iris-pm-dot lit' : 'iris-pm-dot', title: headTitle }, '●') : null,
    ...
    function WorkbenchPanel() {
      ...
      return React.createElement('div', { className: 'iris-wb' },
        ...
        attention.length ? React.createElement('div', {},
          React.createElement('div', { className: 'iris-wb-sec' }, '需要处理'),
          React.createElement('div', { className: 'iris-wb-box' },
            attention.map(function (t) { return taskRow(t, false, selectedTask === t.id, onSelect); }))) : null,
    ...
    function BubblePanel() {
      ...
      var body;
      if (tab === 'tasks') {
        // 快捷面板承载当前任务、需要处理的异常事实与可直接查看的成功产物。
        var succeeded = recent.filter(function (t) {
          return t.status === 'succeeded' && Array.isArray(t.media) && t.media.length > 0;
        });
        var shown = succeeded.slice(0, 6);
        body = React.createElement('div', { className: 'iris-wb-box' },
          ...
          attention.length ? React.createElement('div', {},
            React.createElement('div', { className: 'iris-wb-sec' }, '需要处理'),
            attention.slice(0, 4).map(function (t) { return taskRowMini(t, selectedTask === t.id, onSelect); })) : null,
          React.createElement('div', { className: 'iris-wb-sec' }, '最近完成'),
          shown.length ? shown.map(function (t) { return taskRowMini(t, selectedTask === t.id, onSelect); })
            : React.createElement('div', { className: 'iris-wb-empty' }, '尚无成功产物'),
          succeeded.length > 6 || recentTotal > succeeded.length
            ? React.createElement('div', { className: 'iris-bubble-more', style: { cursor: 'default' } },
              '失败、取消与完整历史请到 Iris 工作台查看') : null);
      } else {
    ...
    function FloatingBubble() {
      ...
      return React.createElement('div', {
        ...
        title: configured ? 'Iris 已就绪：点击打开快捷面板' : 'Iris 尚未配置：点击打开快捷面板'
      },
