  ...
  const submitted = await submitWithFailover(cap.CAPABILITIES.IMAGE, '图像', signal, args.model, async (provider) => {
    ...
    const files = await tasks.submitGuard(task, async () => {
      ...
      for (let i = 0; i < outs.length; i++) {
        ...
        if (outs[i].b64) writePrivateFile(p, Buffer.from(outs[i].b64, 'base64'));
  ...
  const submitted = await submitWithFailover(cap.CAPABILITIES.VIDEO, '视频', signal, args.model, async (provider) => {
    ...
    const remoteTaskId = await tasks.submitGuard(task, async () => {
      ...
      if (isS2V) {
        ...
        const imageUrl = await adapters.uploadTempFile({ key: provider.apiKey, baseUrl: provider.baseUrl, model, filePath: frame.absPath, signal });
        const audioUrl = await adapters.uploadTempFile({ key: provider.apiKey, baseUrl: provider.baseUrl, model, filePath: audioPath, signal });
      ...
      return adapters.submitVideo({ key: provider.apiKey, baseUrl: provider.baseUrl, model, ...submitArgs, signal });
    });
    tasks.update(task.id, { remoteTaskId });
    tasks.watch(tasks.get(task.id), {
      key: () => provider.apiKey,
      poll: ({ key, remoteTaskId: id }) => adapters.pollTask({ key, baseUrl: provider.baseUrl, remoteTaskId: id }),
      intervalMs: 6000,
      onSuccess: async (t, r) => {
        const names = [];
        for (const url of r.urls || []) {
...
register('tts', async (ctx, args, { signal }) => {
  ...
  const submitted = await submitWithFailover(cap.CAPABILITIES.TTS, '语音', signal, args.model, async (provider) => {
    ...
    const fileName = await tasks.submitGuard(task, async () => {
      const r = await adapters.synthesizeTts({ key: provider.apiKey, baseUrl: provider.baseUrl, model, text, voice, signal });
      ...
      else writePrivateFile(p, Buffer.from(r.audioB64, 'base64'));
      return name;
    });
...
register('transcribe', async (ctx, args, { signal }) => {
  ...
  const submitted = await submitWithFailover(cap.CAPABILITIES.TRANSCRIBE, '转写', signal, args.model, async (provider) => {
    ...
    const remoteTaskId = await tasks.submitGuard(task, async () => {
      const audioUrl = await adapters.uploadTempFile({ key: provider.apiKey, baseUrl: provider.baseUrl, model, filePath: audioPath, signal });
      return adapters.submitTranscription({ key: provider.apiKey, baseUrl: provider.baseUrl, model, audioUrl, signal });
    });
    tasks.update(task.id, { remoteTaskId });
    tasks.watch(tasks.get(task.id), {
      key: () => provider.apiKey,
      poll: async ({ key, remoteTaskId: id }) => {
        const r = await adapters.pollTranscriptionTask({ key, baseUrl: provider.baseUrl, remoteTaskId: id });
