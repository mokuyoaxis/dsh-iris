...
function imageRoute(provider, args) {
  ...
  return { provider, protocol, model, modelRef: models.modelRef(provider.id, model) };
}

function imageSubmissionError(error) {
  const status = Number(error && error.status) || undefined;
  // 408/409/425/499 可能来自中间层或冲突恢复，不能证明请求未被受理。
  const explicitRejection = Boolean(status && status >= 400 && status < 500 && ![408, 409, 425, 499].includes(status));
  let category = 'provider';
  if (error && (error.name === 'AbortError' || error.code === 'ABORT_ERR')) category = 'aborted';
  else if (error && (error.category === 'network' || error instanceof TypeError)) category = 'network';
  else if (status === 401 || status === 403) category = 'authentication';
  else if (status === 429 && error && error.category === 'quota') category = 'quota';
  else if (status === 429) category = 'rate_limit';
  else if (status && status >= 400 && status < 500) category = 'invalid_request';
  else if (!status) category = 'protocol';
  return new ProviderContractError(String((error && error.message) || error || '提交失败'), {
    stage: 'submit',
    category,
    acceptance: explicitRejection ? 'not_accepted' : 'unknown',
    retryable: status === 429 || Boolean(status && status >= 500) || category === 'network',
...
async function deliverCompletedImage(taskId, value, signal) {
  const names = [];
  try {
    ...
    if (value && value.kind === 'urls') {
      ...
      for (const item of value.items || []) {
        ...
        else if (item && item.url) await adapters.downloadTo(item.url, target, { signal });
...
async function submitImageV2(routes, prompt, args, signal) {
  ...
  const candidates = routes.map((route) => ({
    ...
    submit: async () => {
      ...
      try {
        if (route.protocol === 'dashscope') {
          if (adapters.dashscopeImageMode(route.model) === 'legacy-async') {
            const remoteTaskId = await adapters.submitImage({
              key: route.provider.apiKey, baseUrl: route.provider.baseUrl,
              model: route.model, prompt, size: args.size, n: args.n, signal
            });
            return { kind: 'accepted', remoteTaskId };
          }
          const urls = await adapters.generateImageMultimodal({
            key: route.provider.apiKey, baseUrl: route.provider.baseUrl,
            model: route.model, prompt, size: args.size, n: args.n, signal
          });
          return { kind: 'completed', value: { kind: 'urls', items: urls } };
        }
        const items = await adapters.openAiGenerateImage({
          key: route.provider.apiKey, baseUrl: route.provider.baseUrl,
          model: route.model, prompt, size: args.size, n: args.n, signal
        });
        return { kind: 'completed', value: { kind: 'openai', items } };
      } catch (error) {
        throw imageSubmissionError(error);
      }
    }
  }));
  ...
  if (submitted.result.kind === 'completed') {
    const delivered = await deliverCompletedImage(task.id, submitted.result.value, signal);
  ...
  tasks.watch(tasks.get(task.id), {
    key: () => route.provider.apiKey,
    poll: ({ key, remoteTaskId }) => adapters.pollTask({ key, baseUrl: route.provider.baseUrl, remoteTaskId }),
    intervalMs: 2500,
...
function videoRoute(provider, args, frame) {
  ...
  return { provider, model, modelRef: models.modelRef(provider.id, model), isS2V, mode, protocol: 'dashscope' };
}
...
async function submitVideoV2(routes, frame, prompt, args, signal) {
  ...
  const candidates = routes.map((route) => ({
    ...
    submit: async (_input, context) => {
      ...
      try {
        const remoteTaskId = await adapters.submitVideo({
          key: route.provider.apiKey, baseUrl: route.provider.baseUrl,
          model: route.model, ...submitArgs, signal
        });
        return { kind: 'accepted', remoteTaskId };
      } catch (error) {
        throw imageSubmissionError(error);
  ...
  tasks.watch(tasks.get(task.id), {
    key: () => route.provider.apiKey,
    poll: ({ key, remoteTaskId: id }) => adapters.pollTask({ key, baseUrl: route.provider.baseUrl, remoteTaskId: id }),
    intervalMs: 6000,
    onSuccess: async (record, result) => {
      const names = [];
      for (const url of result.urls || []) {
...
function simpleModelRoute(provider, args, field, fallback, protocol = 'dashscope') {
  ...
  return { provider, model, modelRef: models.modelRef(provider.id, model), protocol };
}
...
async function submitTranscriptionV2(routes, audioPath, args, signal) {
  ...
  const candidates = routes.map((route) => ({
    ...
    submit: async (_input, context) => {
      ...
      try {
        const remoteTaskId = await adapters.submitTranscription({
          key: route.provider.apiKey, baseUrl: route.provider.baseUrl,
          model: route.model, audioUrl, signal
        });
        return { kind: 'accepted', remoteTaskId };
      } catch (error) {
        throw imageSubmissionError(error);
  ...
  tasks.watch(tasks.get(task.id), {
    ...
    poll: async ({ key, remoteTaskId: id }) => {
      const result = await adapters.pollTranscriptionTask({ key, baseUrl: route.provider.baseUrl, remoteTaskId: id });
      return { done: result.done, ok: result.ok, urls: result.text ? [result.text] : [], message: result.message };
    },
    intervalMs: 2500,
...
async function submitTtsV2(routes, text, args, signal) {
  ...
  const candidates = routes.map((route) => ({
    ...
    submit: async (_input, context) => {
      tasks.markAttemptStage(task.id, context.attempt.id, 'submit');
      try {
        const audio = await adapters.synthesizeTts({
          key: route.provider.apiKey, baseUrl: route.provider.baseUrl,
          model: route.model, text, voice: route.voice, signal
        });
        return { kind: 'completed', value: audio };
      } catch (error) {
        throw imageSubmissionError(error);
      }
    }
  ...
  try {
    ...
    const target = path.join(outputDir(), name);
    if (audio.audioUrl) await adapters.downloadTo(audio.audioUrl, target, { signal });
...
export function taskPollDeps(provider, task) {
  ...
  if (task.cap === 'transcribe') {
    return {
      ...
      poll: async ({ key, remoteTaskId }) => {
        const r = await adapters.pollTranscriptionTask({ key, baseUrl: provider.baseUrl, remoteTaskId });
  ...
  if (task.cap !== 'image' && task.cap !== 'video') return null;
  return {
    key: () => provider.apiKey,
    poll: ({ key, remoteTaskId }) => adapters.pollTask({ key, baseUrl: provider.baseUrl, remoteTaskId }),
...
/* ---------- Task v2 人工接管 ---------- */
register('task_reobserve', async (ctx, args) => {
...
register('task_redeliver', async (ctx, args, { signal }) => {
  const id = String(args.task_id || '').trim();
  const task = tasks.get(id);
  ...
  try {
    ...
    const result = await deps.poll({ key: deps.key(), remoteTaskId: task.remoteTaskId, signal });
...
register('task_ack_attention', async (_ctx, args) => {
  const id = String(args.task_id || '').trim();
  const task = tasks.get(id);
...
register('task_restore_attention', async (_ctx, args) => {
  const id = String(args.task_id || '').trim();
  const task = tasks.get(id);
...
register('task_manual_retry', async (ctx, args, { signal }) => {
  const id = String(args.task_id || '').trim();
  ...
  try {
    const result = await runAction(ctx, action, retryArgs, { signal });
...
async function probeModel(ctx, p, modelId, capability) {
  ...
  if (capability === cap.CAPABILITIES.TTS) {
    const r = await adapters.synthesizeTts({ key: p.apiKey, baseUrl: p.baseUrl, model: modelId, text: '你好', voice: 'Cherry' });
    const ok = !!(r.audioUrl || r.audioB64);
    return { ok, note: ok ? '合成返回音频' : '无音频返回' };
  }
  if (capability === cap.CAPABILITIES.IMAGE) {
    if ((p.mediaProtocol || inferMediaProtocol(p.baseUrl)) === 'openai-images') {
    ...
    const started = await adapters.startImageGeneration({ key: p.apiKey, baseUrl: p.baseUrl, model: modelId, prompt: '红色圆形', size: '512*512', n: 1 });
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const st = await adapters.pollTask({ key: p.apiKey, baseUrl: p.baseUrl, remoteTaskId: started.remoteTaskId });
