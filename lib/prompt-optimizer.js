...
/**
 * 执行一次显式的提示词优化。
 * @param {object} host Iris Host Adapter
...
export async function optimizePrompt(host, input, { signal } = {}) {
  ...
  const route = resolvePromptOptimizerRoute(host, config, input && input.route);
  const llm = requireHostPort(host, 'textModel', '提示词优化');
  if (typeof llm.stream !== 'function') throw new Error('Iris Host textModel 缺少 stream，无法优化提示词');
