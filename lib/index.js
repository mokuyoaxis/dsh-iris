 ...
 */
function resolveFirstFrame(args) {
  if (args.first_frame_attachment_id) {
    ...
    return toDataUrl(hit.absPath, hit.mediaType);
  }
...
export async function apply(ctx) {
  ...
  ctx.effect(() => tools.register({
    ...
    description:
      'Generate a video with the configured DashScope video model (wan* t2v/i2v). Text-to-video from a prompt, or image-to-video by passing a first frame ' +
      '(an attachment id returned by iris_draw_image, or an absolute local image path). Submission returns fast; the render is watched in the background — ' +
      'if it takes longer than the inline wait, you get the task id and can check progress later with iris_task_status.',
    parameters: {
      prompt: { type: 'string', required: true, description: 'Detailed description of the motion/scene for the video' },
      first_frame_attachment_id: { type: 'string', description: 'Attachment id of an image generated by iris_draw_image to use as the first frame (image-to-video)' },
      ...
      size: { type: 'string', description: "Output size 'W*H', e.g. '1280*720' (default), '960*960', '720*1280'" },
      duration: { type: 'number', description: 'Duration in seconds, if the model supports it' },
    ...
    async execute(args, exec) {
      ...
      const size = args.size ? String(args.size) : '1280*720';
      const duration = args.duration ? Number(args.duration) : undefined;
      if (exec.signal && exec.signal.aborted) throw new Error('已取消');

      const imgDataUrl = resolveFirstFrame(args); // 参数不合法会直接抛人话错误
      const task = tasks.create({
        cap: 'video', providerId: provider.id, providerName: provider.name,
        model, size, ...(duration ? { duration } : {}),
        mode: imgDataUrl ? 'i2v' : 't2v',
        prompt: prompt.slice(0, 4000)
      });

      let remoteId;
      try {
        remoteId = await adapters.submitVideo({
          key: provider.apiKey, model, prompt,
          imgDataUrl: imgDataUrl || undefined, size, duration
        });
      } catch (err) {
        tasks.update(task.id, { status: 'failed', error: String(err.message || err) });
        throw err;
      ...
      return `[iris] 视频已生成（${model}${final.mode === 'i2v' ? '，图生视频' : ''}，约 ${secs}s）：\n` +
        paths.join('\n') +
        `\ntask: ${final.id}\n视频为 mp4 文件，可直接用系统播放器打开。`;
    }
  }));
