...
import { serveApi, closeAllSse } from './api.js';
...
function mountIrisRoutes(routeCtx) {
  ...
  try {
    ...
    const disposeApi = reg.register({
      kind: 'prefix',
      path: '/iris/api',
      handler: (req, res) => serveApi(req, res, routeCtx)
    });
...
export async function apply(ctx) {
  ...
  safeRegister(ctx, {
    ...
    description: 'Transcribe an audio file (wav/mp3) to text using the configured DashScope provider (qwen-audio-turbo). The audio is uploaded to Bailian temp storage, then an async task is submitted. Returns the full recognized text.',
    parameters: {
      type: 'object',
      properties: {
        audio_path: { type: 'string', description: 'Absolute path to the audio file (wav/mp3)' },
        model: { type: 'string', description: 'Override the transcription model (default qwen-audio-turbo)' }
      },
      required: ['audio_path']
    },
    output: { schema: { type: 'string' }, render: (_args, v) => [{ type: 'text', text: v }] },
    ...
    async execute(args, exec) {
      ...
      tasks.watch(tasks.get(task.id), {
        ...
        intervalMs: 2500,
        onSuccess: async (t, r) => {
          const text = (r.urls && r.urls.join('')) || '';
          tasks.update(t.id, { status: 'succeeded', transcribeText: text, finishedAt: new Date().toISOString() });
          return [];
        }
      });
      const final = await awaitTerminal(task.id, { timeoutMs: 120000, signal: exec.signal });
      if (!final) return `[iris] 音频转写任务已提交并转入后台（task: ${task.id}）。完成后用 iris_task_status 查询。`;
      if (final.status === 'canceled') throw new Error('已取消');
      if (final.status !== 'succeeded') throw new Error('转写失败：' + (final.error || '未知原因'));
      return `[iris] 音频转写完成（${model}）：\n${final.transcribeText || ''}`;
    }
  });
  ...
  safeRegister(ctx, {
    ...
    description:
      ...
      'Requires ffmpeg + ffprobe on PATH. Transcription reuses the configured provider (qwen-audio-turbo) when the video has an audio track.',
    parameters: {
      type: 'object',
      properties: {
        video_path: { type: 'string', description: 'Absolute path to the local video file (mp4/webm/mov etc.)' },
      ...
      if (wantTranscribe) {
        ...
        if (provider) {
          try {
            ...
            try {
              const model = provider.ttsModel ? String(provider.ttsModel).replace(/tts.*/i, 'audio-turbo') : 'qwen-audio-turbo';
              const task = tasks.create({ cap: 'transcribe', providerId: provider.id, providerName: provider.name, model, prompt: 'summarize audio: ' + path.basename(p) });
              const remoteTaskId = await tasks.submitGuard(task, async () => {
                const audioUrl = await adapters.uploadTempFile({ key: provider.apiKey, model, filePath: audio.filePath });
                return adapters.submitTranscription({ key: provider.apiKey, model, audioUrl });
              });
              tasks.update(task.id, { remoteTaskId });
              tasks.watch(tasks.get(task.id), {
                key: () => provider.apiKey,
                poll: async ({ key, remoteTaskId: rtid }) => {
                  const r = await adapters.pollTranscriptionTask({ key, remoteTaskId: rtid });
                  return { done: r.done, ok: r.ok, urls: r.text ? [r.text] : [], message: r.message };
                },
                intervalMs: 2500,
                onSuccess: async (t, r) => {
              ...
              else if (!final) transcriptNote = `\n（音轨转写仍在后台：task ${task.id}，本次摘要未含语音内容）`;
            } finally {
