...
export async function apply(ctx) {
  ...
  safeRegister(ctx, {
    ...
    parameters: {
      ...
      properties: {
        ...
        question: { type: 'string', required: true, description: 'New question about the image pixels' },
  ...
  console.log('[iris] tools registered: iris_draw_image, iris_generate_video, iris_speak_text, iris_task_status, iris_look_at_image, iris_relook_attachment');
};
...
/**
 * 看图问答（M3 降级链）：
 *   ① 自持栈为主：providers.json 里的 vision 供应商（OpenAI 兼容，qwen-vl）走 SSE 流式
 *   ② 全局视觉模型为辅：ctx.llm 注册表（默认视觉能力模型）
 * @returns {{answer:string,via:'selfstack'|'global',model?:string}}
 */
export async function askVision(ctx, { question, ref, dataUrl, signal, model }) {
  ...
  // ① 自持栈（主）：OpenAI 兼容协议才能走 chat/completions
  const provider = store.pickFor('vision');
  if (provider && provider.type === 'openai') {
    try {
      const m = String(model || provider.visionModel || 'qwen-vl-plus');
      const answer = await adapters.visionStream({
        key: provider.apiKey,
      ...
      if (answer && answer.trim()) return { answer: answer.trim(), via: 'selfstack', model: m };
    } catch (_) {
      /* 降级到全局 */
    }
  }
  ...
  if (llm && typeof llm.stream === 'function') {
    try {
      ...
      if (answer) return { answer, via: 'global', model: '全局视觉模型' };
    } catch (_) {
      /* 两端都失败再抛 */
    }
  }

  throw new Error('iris: 视觉模型不可用——自持栈与全局视觉模型都失败了');
}
