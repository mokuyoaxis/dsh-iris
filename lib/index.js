...
export async function apply(ctx) {
  ...
  safeRegister(ctx, {
    ...
    async execute(args, exec) {
      ...
      return `[iris] 裁剪完成：${width}x${height}（原区域 ${args.left},${args.top},${args.width},${args.height}）\nattachment: ${ref.attachmentId}`;
    }
  });
  ...
  safeRegister(ctx, {
    ...
    async execute(args, exec) {
      ...
      return `[iris] 像素差异 ${pct}%（${diffPixels}/${totalPixels}px，归一化 ${width}x${height}）\n` +
  ...
  safeRegister(ctx, {
    ...
    async execute(args, exec) {
      ...
      return `[iris] HTML 截图完成（${outW}x${outH}，fullPage: ${args.fullPage !== false}）\n` +
        `attachment: ${ref.attachmentId}` +
        `\n注意：渲染在共享浏览器临时标签页中完成，页面短暂可见属正常行为。`;
    }
  });

  console.log('[iris] tools registered: iris_draw_image, iris_generate_video, iris_speak_text, iris_task_status, iris_look_at_image, iris_relook_attachment, iris_crop, iris_pixel_diff, iris_locate, iris_html_screenshot');
};
