...
import { buildVisionBackendsFromHost, askWithBackends } from './vision.js';
import { hasHostPort, requireHostPort } from './host-contract.js';
import { createDshHostAdapter } from './dsh-host-adapter.js';
import { cropForDsh, inspectProviderTaskForDsh, projectCoreTaskForDsh, readCoreArtifactMediaForDsh, resumeProviderTaskWatchesForDsh, stopProviderTaskWatchesForDsh } from './dsh-core-adapter.js';
...
export async function apply(ctx) {
    ...
    async execute(args, exec) {
      ...
      if (submitted.storage === 'core') {
        ...
        if (!coreTaskWaitFinished(final)) {
          final = await awaitCoreTerminal(submitted.taskId, { timeoutMs: AWAIT_MS.image, signal: exec.signal });
        }
        if (!final) {
          return { blocks: [{ type: 'text', text: `[iris] Core 图片任务仍在后台观察（task: ${submitted.taskId}）。稍后可用 iris_task_status 查询进度与 Artifact。` }] };
        }
        const terminalProblem = coreTaskTerminalProblem(final, '图像生成');
        if (terminalProblem) throw new Error(terminalProblem);
        const projected = await projectCoreTaskForDsh(dshHost(ctx), submitted.taskId, { signal: exec.signal });
        ...
        let note = '';
        try {
          const first = await readCoreArtifactMediaForDsh(projected.artifacts[0].artifact.id, { signal: exec.signal });
          const described = await describeImageData(dshHost(ctx), refs[0],
            `data:${first.artifact.mediaType};base64,${first.bytes.toString('base64')}`, prompt);
          if (described) note = `画面内容：${described}`;
        } catch (_) { /* 自述是增强项，失败不影响 Core 交付 */ }
      ...
      let note = '';
      try {
        const described = await describeFirstImage(dshHost(ctx), refs[0], files[0], prompt);
        if (described) note = `画面内容：${described}`;
      } catch (_) { /* 增强失败不影响生成结果 */ }
      blocks.unshift({
        type: 'text',
  ...
  safeRegister(host, {
    ...
    isConcurrencySafe: () => false,
    async execute(args, exec) {
      const submitted = await runAction(dshHost(ctx), 'video', args, { signal: exec.signal });
      if (submitted.storage === 'core') {
        let final = await inspectProviderTaskForDsh(submitted.taskId, { signal: exec.signal });
        if (!coreTaskWaitFinished(final)) {
  ...
  safeRegister(host, {
    ...
    async execute(args, exec) {
      ...
      if (wantTranscribe) {
        if (store.pickAllFor(cap.CAPABILITIES.TRANSCRIBE).length) {
          try {
            ...
            try {
              const submitted = await runAction(dshHost(ctx), 'transcribe', { audio_path: audio.filePath }, { signal: exec.signal });
              const final = await awaitTerminal(submitted.taskId, { timeoutMs: 120000, signal: exec.signal });
      ...
      const backends = buildVisionBackendsFromHost(dshHost(ctx), { providers: visionProviders, model: args.model });
  ...
  safeRegister(host, {
    ...
    async execute(args, exec) {
      ...
      const ref = await attachmentService(host).saveImage({ data: new Uint8Array(fs.readFileSync(p)), mediaType: mt, name: path.basename(p) });
      return runVisionTool(dshHost(ctx), exec, { origin: 'tool', model: args.model, question, ref, dataUrl });
    }
  });
  ...
  safeRegister(host, {
    ...
    async execute(args, exec) {
      ...
      if (hit.absPath) {
        ...
        const stored = await attachmentService(host).readImage(hit.ref, exec.signal);
        dataUrl = `data:${stored.mediaType || hit.ref.mediaType};base64,${Buffer.from(stored.data).toString('base64')}`;
      }
      return runVisionTool(dshHost(ctx), exec, { origin: 'relook', model: args.model, question, ref: hit.ref, dataUrl });
    }
  });
  ...
  safeRegister(host, {
    ...
    async execute(args, exec) {
      ...
      const rendered = await browser.renderHtml({
        html: args.html,
        width: args.width,
        height: args.height,
  ...
  safeRegister(host, {
    ...
    async execute(args, exec) {
      ...
      const visionProviders = store.pickAllFor(cap.CAPABILITIES.VISION);
      const backends = buildVisionBackendsFromHost(dshHost(ctx), { providers: visionProviders });
 ...
 */
export async function askVision(host, { question, ref, dataUrl, signal, model }) {
  const visionProviders = store.pickAllFor(cap.CAPABILITIES.VISION);
  const backends = buildVisionBackendsFromHost(host, { providers: visionProviders, model });
 ...
 */
export async function runVisionTool(host, exec, { origin, model, question, ref, dataUrl }) {
...
async function describeFirstImage(host, ref, absPath, originalPrompt) {
  try {
    ...
    return await describeImageData(host, ref, toDataUrl(absPath, mt), originalPrompt);
  } catch (_) {
    return ''; // 自述是增强项，失败不影响交付
  }
}

async function describeImageData(host, ref, dataUrl, originalPrompt) {
  const { answer } = await askVision(host, {
    question: `用不超过两句话描述这张图片的主题与构图。生成该图的提示词是：「${String(originalPrompt).slice(0, 200)}」`,
    ref, dataUrl, signal: undefined
  });
