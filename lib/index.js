...
export async function apply(ctx) {
  ...
  ctx.effect(() => tools.register({
    ...
    async execute(args, exec) {
      ...
      const ref = await ctx.attachments.saveImage({ data: new Uint8Array(fs.readFileSync(p)), mediaType: mt, name: path.basename(p) });
  ...
  ctx.effect(() => tools.register({
    ...
    async execute(args, exec) {
      ...
      const hit = sessionAttachmentRef(exec, id) || findOwnAttachment(id);
      ...
      if (hit.absPath) {
        ...
        const stored = await ctx.attachments.readImage(hit.ref, exec.signal);
