...
import { dashscopeApiBase, isDashScopeBaseUrl } from './provider-protocol.js';
import { chmodPrivateFile, ensurePrivateDir, privateSibling } from './private-storage.js';
...
export async function generateImageMultimodal({ key, baseUrl, model, prompt, size, n = 1, timeoutMs = T_SYNC_GEN, signal }) {
  const apiBase = dashscopeApiBase(baseUrl);
  const res = await fetch(`${apiBase}/services/aigc/multimodal-generation/generation`, {
    method: 'POST',
...
export async function submitImage({ key, baseUrl, model, prompt, size, n = 1, timeoutMs = T_SUBMIT, signal }) {
  const apiBase = dashscopeApiBase(baseUrl);
  const res = await fetch(`${apiBase}/services/aigc/text2image/image-synthesis`, {
    method: 'POST',
 ...
 */
export async function submitVideo({ key, baseUrl, model, prompt, imgDataUrl, size = '1280*720', duration, audioUrl, resolution, timeoutMs = T_SUBMIT, signal }) {
  const apiBase = dashscopeApiBase(baseUrl);
  const isS2V = /s2v/i.test(model || '');
  let url, input, parameters;
  const headers = { ...authHeaders(key), 'X-DashScope-Async': 'enable' };
  if (isS2V) {
    url = `${apiBase}/services/aigc/image2video/video-synthesis`;
    ...
    url = `${apiBase}/services/aigc/video-generation/video-synthesis`;
    input = { prompt };
 ...
 */
export async function uploadTempFile({ key, baseUrl, model, filePath, timeoutMs = T_UPLOAD, signal }) {
  const apiBase = dashscopeApiBase(baseUrl);
  const u = new URL(`${apiBase}/uploads`);
...
export async function pollTask({ key, baseUrl, remoteTaskId, timeoutMs = T_POLL, signal }) {
  const apiBase = dashscopeApiBase(baseUrl);
  const res = await fetch(`${apiBase}/tasks/${encodeURIComponent(remoteTaskId)}`, {
    headers: authHeaders(key),
    signal: timeoutSignal(timeoutMs, signal)
  });
  const body = await readJson(res);
...
export async function synthesizeTts({ key, baseUrl, model = 'qwen-tts-latest', text, voice = 'Cherry', timeoutMs = T_SYNC_GEN, signal }) {
  const apiBase = dashscopeApiBase(baseUrl);
  const res = await fetch(`${apiBase}/services/aigc/multimodal-generation/generation`, {
    method: 'POST',
    headers: authHeaders(key),
...
/* ---------------- 模型发现（OpenAI GET /models + DashScope GET /api/v1/models） ---------------- */
/**
 * 拉取供应商模型目录。DashScope 官方端点分页读取并保留 capabilities；
 * 普通 OpenAI 兼容端点维持 GET /models 与字符串 id 返回形状。
 * @returns {Promise<Array<string|{id:string,capabilities?:string[]}>>}
 */
export async function listModels({ key, baseUrl, timeoutMs = T_SUBMIT, signal }) {
  const base = String(baseUrl || '').trim().replace(/\/+$/, '');
  if (!base) throw new Error('iris: 供应商缺 baseUrl，无法发现模型');
  const dashscope = isDashScopeBaseUrl(base);
  const endpoint = dashscope ? dashscopeApiBase(base) + '/models' : base + '/models';
  const found = new Map();
  const maxPages = dashscope ? 50 : 1;
  let loaded = 0;
  for (let page = 1; page <= maxPages; page++) {
    ...
    const res = await fetch(url, { headers: authHeaders(key), signal: timeoutSignal(timeoutMs, signal) });
    const body = await readJson(res);
    if (!res.ok) throw providerError(res.status, body, '模型发现失败');
    const output = body && body.output;
    const items = (output && Array.isArray(output.models) && output.models)
    ...
    for (const item of items) {
      ...
      const capabilities = Array.isArray(item?.capabilities)
        ? item.capabilities.filter((value) => typeof value === 'string' && value)
        : [];
      const requestModalities = Array.isArray(item?.inference_metadata?.request_modality)
        ? item.inference_metadata.request_modality.map((value) => String(value).toLowerCase()) : [];
      const responseModalities = Array.isArray(item?.inference_metadata?.response_modality)
        ? item.inference_metadata.response_modality.map((value) => String(value).toLowerCase()) : [];
      if (responseModalities.includes('image')) capabilities.push('IG');
      if (responseModalities.includes('video')) capabilities.push('VG');
      if (requestModalities.includes('audio') && responseModalities.includes('text')) capabilities.push('ASR');
      if (requestModalities.includes('text') && responseModalities.includes('audio')) capabilities.push('TTS');
      if (requestModalities.includes('image') && responseModalities.includes('text')) capabilities.push('VU');
      const normalizedCapabilities = [...new Set(capabilities)];
      found.set(id, normalizedCapabilities.length ? { id, capabilities: normalizedCapabilities } : id);
    }
    if (!dashscope) break;
    const statedTotal = Number(output?.total);
    const total = Number.isFinite(statedTotal) && statedTotal >= 0 ? statedTotal : null;
    if (!items.length || (total !== null && loaded >= total) || items.length < 100) break;
    if (page === maxPages) throw new Error('iris: 模型目录超过安全分页上限，已保留原模型池');
  }
  return [...found.values()];
}
...
export async function downloadTo(url, filePath, { timeoutMs = T_DOWNLOAD, signal } = {}) {
  ...
  if (!res.body) throw new Error('下载结果失败：响应没有内容');
  ensurePrivateDir(path.dirname(filePath));
  const tmp = privateSibling(filePath, 'download');
  try {
    ...
    try { fs.rmSync(tmp, { force: true }); } catch (_) { /* rename 成功后已不存在 */ }
  }
}
 ...
 */
export async function submitTranscription({ key, baseUrl, model = 'qwen-audio-3.0-asr-flash-filetrans', audioUrl, timeoutMs = T_SUBMIT, signal }) {
  const apiBase = dashscopeApiBase(baseUrl);
  const headers = { ...authHeaders(key), 'X-DashScope-Async': 'enable' };
  ...
  const input = /^qwen3-asr-.*filetrans/i.test(model || '')
    ? { file_url: audioUrl }
    : { file_urls: [audioUrl] };
  const res = await fetch(`${apiBase}/services/audio/asr/transcription`, {
    method: 'POST',
    headers,
    signal: timeoutSignal(timeoutMs, signal),
...
export async function pollTranscriptionTask({ key, baseUrl, remoteTaskId, timeoutMs = T_POLL, signal }) {
  const apiBase = dashscopeApiBase(baseUrl);
  const res = await fetch(`${apiBase}/tasks/${encodeURIComponent(remoteTaskId)}`, {
    headers: authHeaders(key),
    signal: timeoutSignal(timeoutMs, signal)
  ...
  if (Array.isArray(out.results)) {
    for (const result of out.results) {
      ...
      if (status === 'SUCCEEDED' && result.subtask_status !== 'FAILED' && typeof result.transcription_url === 'string') {
        const transcriptRes = await fetch(result.transcription_url, { signal: timeoutSignal(timeoutMs, signal) });
