...
/* ---------------- DashScope：图像（异步） ---------------- */
export async function submitImage({ key, model, prompt, size = '1024*1024', n = 1, timeoutMs = T_SUBMIT }) {
 ...
 */
export async function submitVideo({ key, model, prompt, imgDataUrl, size = '1280*720', duration, audioUrl, resolution, timeoutMs = T_SUBMIT }) {
 ...
 */
export async function uploadTempFile({ key, model, filePath, timeoutMs = T_UPLOAD }) {
  const u = new URL(`${DASHSCOPE}/uploads`);
  u.searchParams.set('action', 'getPolicy');
  u.searchParams.set('model', model);
  const pres = await fetch(u, { headers: authHeaders(key), signal: timeoutSignal(timeoutMs) });
  ...
  const up = await fetch(pol.upload_host, { method: 'POST', body: fd, signal: timeoutSignal(timeoutMs) });
...
export async function synthesizeTts({ key, model = 'qwen-tts-latest', text, voice = 'Cherry', timeoutMs = T_SYNC_GEN }) {
...
export async function openAiGenerateImage({ key, baseUrl, model, prompt, size = '1024x1024', n = 1, timeoutMs = T_SYNC_GEN }) {
...
export async function downloadTo(url, filePath, { timeoutMs = T_DOWNLOAD } = {}) {
 ...
 */
export async function submitTranscription({ key, model = 'qwen-audio-turbo', audioUrl, timeoutMs = T_SUBMIT }) {
  const res = await fetch(`${DASHSCOPE}/services/audio/asr/transcription`, {
    method: 'POST',
    headers: { ...authHeaders(key), 'X-DashScope-Async': 'enable' },
    signal: timeoutSignal(timeoutMs),
    body: JSON.stringify({ model, input: { audio_url: audioUrl } })
  });
  const body = await readJson(res);
...
/** 轮询 ASR 转写结果，从 results 中提取 transcription_text */
export async function pollTranscriptionTask({ key, remoteTaskId, timeoutMs = T_POLL }) {
  ...
  if (Array.isArray(out.results)) {
    for (const r of out.results) {
      if (r && typeof r.transcription_text === 'string') text += r.transcription_text;
