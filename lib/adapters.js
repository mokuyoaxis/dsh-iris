...
export async function pollTask({ key, baseUrl, remoteTaskId, timeoutMs = T_POLL }) {
  const apiBase = dashscopeApiBase(baseUrl);
...
export async function pollTranscriptionTask({ key, baseUrl, remoteTaskId, timeoutMs = T_POLL }) {
  const apiBase = dashscopeApiBase(baseUrl);
  const res = await fetch(`${apiBase}/tasks/${encodeURIComponent(remoteTaskId)}`, {
    headers: authHeaders(key),
    signal: timeoutSignal(timeoutMs)
