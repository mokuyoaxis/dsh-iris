...
import { providerPollTaskView } from './provider-adapter.js';
...
/** 终态或已显式处理提醒的 Task v2 可删；活跃/未处理任务仍受保护。 */
export function taskRecordRemovable(task) {
  if (!task) return false;
  if (task.status !== 'running') return true;
  return isV2Task(task)
    && ATTENTION_USER_STATES.has(deriveUserState(task))
    && attentionDisposition(task).status === 'acknowledged';
}

/**
 * 删除单条任务元数据。事实终态可直接删除；状态未知只有在用户已读/已重试归档后才可删除。
...
export function remove(id) {
  ...
  if (!taskRecordRemovable(t)) return { ok: false, reason: '活跃或待确认任务请先取消、完成提醒处置，或标为已读后再删除' };
  stopWatch(id);
...
export function prune(pred) {
  ...
  const kept = [];
  for (const t of arr) {
    if (taskRecordRemovable(t) && pred(t)) {
      stopWatch(t.id);
      removed.push(t.id);
    } else kept.push(t);
  }
 ...
 * @param {object} deps
 *   key?(): string                                   —— 仅供旧 poll 兼容；v0 Adapter 自持凭据闭包
 *   poll({key?, remoteTaskId}): Provider poll v0（兼容旧 done/ok 形状）—— 协议适配器查询
  ...
  const tick = async () => {
    ...
    try {
      const raw = await deps.poll({
        key: typeof deps.key === 'function' ? deps.key() : undefined,
        remoteTaskId: t.remoteTaskId
      });
      r = providerPollTaskView(raw);
    } catch (err) {
    ...
    if (!r.ok) {
      ...
      if (!v2) {
        return finishLegacy({ status: r.kind === 'canceled' ? 'canceled' : 'failed', error: message });
      }
      if (r.kind === 'unknown') {
        return finishV2({
          phase: 'terminal', watchState: 'exhausted', outcome: 'unknown',
          lastError: r.error || providerErrorRecord(message, {
      ...
      if (r.kind === 'canceled') {
        return finishV2({
          phase: 'terminal', watchState: 'idle', outcome: 'canceled', cancelState: 'remote_confirmed',
          lastError: r.error, error: message, finishedAt: new Date().toISOString()
        });
      }
      return finishV2({
