...
export function defineProviderAdapter(input) {
  ...
  const allowed = new Set([
    ...
    'prepareInput', 'unsupportedPreparation'
  ]);
  ...
  if (input.prepareInput !== undefined && typeof input.prepareInput !== 'function') {
    throw new TypeError('Provider prepareInput 必须是函数');
  }
  if (input.prepareInput && input.unsupportedPreparation !== undefined) {
    throw new Error('Provider 输入准备不能同时声明 supported 与 unsupported');
  }
  const unsupportedPreparation = input.prepareInput ? undefined
    : normalizedReason(input.unsupportedPreparation, '协议未实现临时输入上传');
...
export function providerAdapterSnapshot(adapter) {
  ...
  return Object.freeze({
    ...
    operations: Object.freeze(operations),
    inputPreparation: Object.freeze(typeof adapter?.prepareInput === 'function'
      ? { status: 'supported' }
      : { status: 'unsupported', reason: adapter?.unsupportedPreparation || '协议未实现临时输入上传' })
  });
}

/** 提交前的可选上传能力；不创建 Task，也不属于六个生命周期操作。 */
export async function prepareProviderInput(adapter, input = {}) {
  if (typeof adapter?.prepareInput !== 'function') {
    const error = new ProviderContractError(adapter?.unsupportedPreparation || '协议未实现临时输入上传', {
      stage: 'upload', category: 'invalid_request', acceptance: 'not_accepted',
      providerCode: 'IRIS_PROVIDER_INPUT_PREPARATION_UNSUPPORTED'
    });
    error.code = 'IRIS_PROVIDER_INPUT_PREPARATION_UNSUPPORTED';
    throw error;
  }
  try {
    const value = await adapter.prepareInput(input);
    if (typeof value?.url !== 'string' || !value.url.trim()) {
      throw new ProviderContractError('Provider 输入准备必须返回非空 url', {
        stage: 'upload', category: 'protocol', acceptance: 'not_accepted'
      });
    }
    // 临时地址只供随后的提交使用，避免随日志或任务 JSON 序列化。
    return Object.freeze(Object.defineProperty({}, 'url', { value: value.url }));
  } catch (error) {
    const context = { stage: 'upload', acceptance: 'not_accepted' };
    const safe = providerErrorRecord(error instanceof ProviderContractError
      ? error : mappedError(adapter, error, context), context);
...
export function normalizeProviderPollResult(value) {
  ...
  if (!PROVIDER_POLL_KINDS.includes(value.kind) && typeof value.done === 'boolean') {
    ...
    if (value.ok) {
      return Object.freeze({
        kind: 'succeeded',
        artifacts: normalizeProviderArtifacts((value.urls || []).map((url) => ({ kind: 'remote-url', url }))),
  ...
  if (value.kind === 'succeeded') {
    return Object.freeze({
      kind: 'succeeded',
      artifacts: normalizeProviderArtifacts(value.artifacts),
...
export function providerPollTaskView(value) {
  ...
  if (result.kind === 'succeeded') {
    const urls = result.artifacts.filter((item) => item.kind === 'remote-url').map((item) => item.url);
