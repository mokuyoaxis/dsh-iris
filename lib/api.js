...
const MAX_UPLOAD_BYTES = 64 * 1024 * 1024; // 64MB（视频/音频），独立于 actions 的 1MB JSON 上限
function uploadsDir() {
  return path.join(irisHome(), 'uploads');
}
/** 只保留安全文件名字符，去路径分隔/穿越；空则回退 upload */
function safeName(name) {
  const base = path.basename(String(name || '')).replace(/[^\w.\-一-鿿]/g, '_').slice(0, 120);
...
function handleUpload(req, res) {
  ...
  req.on('data', (c) => {
    if (overflow) return;
    totalBytes += c.length;
    if (totalBytes > MAX_UPLOAD_BYTES) { overflow = true; chunks.length = 0; return; }
    chunks.push(c);
  });
  req.on('end', () => {
    if (overflow) return sendJson(res, 413, { error: 'file too large (limit 64MB)' });
    if (!totalBytes) return sendJson(res, 400, { error: 'empty upload' });
    try {
      fs.mkdirSync(uploadsDir(), { recursive: true });
      const abs = path.join(uploadsDir(), Date.now().toString(36) + '-' + name);
      fs.writeFileSync(abs, Buffer.concat(chunks), { mode: 0o600 });
      sendJson(res, 200, { ok: true, path: abs, name });
    } catch (err) {
      sendJson(res, 500, { error: 'upload failed: ' + String(err && err.message) });
    }
  });
  req.on('error', () => sendJson(res, 400, { error: 'bad upload' }));
}
