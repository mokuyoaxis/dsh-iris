...
export function serveApi(req, res, ctx) {
  try {
    ...
    if (method === 'POST') {
      ...
      const promptMatch = typeof ctx === 'object' && urlPath.match(/^\/iris\/api\/prompt-optimizer\/(optimize|import|reset|enabled)\/?$/);
      if (promptMatch) return handlePromptOptimizer(ctx, promptMatch[1], req, res);
...
function handleAction(ctx, name, req, res) {
  return handleJsonPost(req, res, (args, signal) => runAction(ctx, name, args, { signal }));
}

function handlePromptOptimizer(ctx, operation, req, res) {
  return handleJsonPost(req, res, async (body, signal) => {
    if (operation === 'optimize') return optimizePrompt(ctx, body, { signal });
    if (operation === 'import') {
