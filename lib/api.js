...
function handleUpload(req, res) {
  ...
  const tmp = abs + '.part';
  const out = fs.createWriteStream(tmp, { mode: 0o600, flags: 'wx' });
  let totalBytes = 0;
  let settled = false;
  ...
  req.on('data', (chunk) => {
    ...
    if (!out.write(chunk) && typeof req.pause === 'function') {
      req.pause();
      out.once('drain', () => { if (!settled && typeof req.resume === 'function') req.resume(); });
    }
  });
  req.on('end', () => {
    if (settled) return;
