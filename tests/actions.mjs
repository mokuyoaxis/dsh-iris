...
/* ① POST /status */
const r1 = fakeRes();
serveApi(fakeReq('POST', '/iris/api/actions/status', '{}'), r1, stubHost);
await new Promise((r) => setTimeout(r, 50));
assert(r1.status === 200 && r1.body && r1.body.ok, 'POST status 200 + ok', r1.body && r1.body.text);

/* ② POST 不存在动作 */
const r2 = fakeRes();
serveApi(fakeReq('POST', '/iris/api/actions/nonexist', '{}'), r2, stubHost);
await new Promise((r) => setTimeout(r, 50));
assert(r2.status === 400, '不存在动作 400', r2.status);

/* ③ GET → 404（该路径只支持 POST，GET 落到 404 not found） */
const r3 = fakeRes();
serveApi(fakeReq('GET', '/iris/api/actions/status', null), r3, stubHost);
assert(r3.status === 404, 'GET 404（只支持 POST）', r3.status);

/* ④ POST 空 body */
const r4 = fakeRes();
serveApi(fakeReq('POST', '/iris/api/actions/status', ''), r4, stubHost);
await new Promise((r) => setTimeout(r, 50));
assert(r4.status === 200 && r4.body && r4.body.ok, '空 body 也接受', r4.body);

/* ④b POST 超 1MB body → 413（按字节计：3×500KB 块 = 1.5MB，旧实现按块数会放行） */
const r4b = fakeRes();
serveApi(fakeReqChunks('POST', '/iris/api/actions/status', [Buffer.alloc(500000, 0x61), Buffer.alloc(500000, 0x61), Buffer.alloc(500000, 0x61)]), r4b, stubHost);
await new Promise((r) => setTimeout(r, 50));
assert(r4b.status === 413, '超限 body → 413', r4b.status);
/* ④c 限内多块 body 正常放行 */
const r4c = fakeRes();
serveApi(fakeReqChunks('POST', '/iris/api/actions/status', [Buffer.from('{"a":'), Buffer.from('1}')]), r4c, stubHost);
await new Promise((r) => setTimeout(r, 50));
...
const listed = await runAction(stubHost, 'providers_list', {});
...
await runAction(stubHost, 'providers_upsert', { name: '新供应商', baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1', apiKey: 'sk-new-key', imageModel: 'wan2.2-t2i-flash', videoModel: 'wan2.2-t2v-flash' });
config.resetCache();
const listed2 = await runAction(stubHost, 'providers_list', {});
...
await runAction(stubHost, 'providers_upsert', { id: np.id, enabled: false });
...
await runAction(stubHost, 'providers_upsert', { id: np.id, enabled: true });

// ③ set_models 覆盖模型池
await runAction(stubHost, 'providers_set_models', { id: np.id, models: [{ id: 'gpt-image-1', capabilities: ['image-gen'] }] });
config.resetCache();
const listed3 = await runAction(stubHost, 'providers_list', {});
...
const tv = await runAction(stubHost, 'providers_test_vision', { id: np.id });
assert(tv.ok && tv.tested === false && /未配置 vision/.test(tv.text), 'test_vision 无 vision 跳过', tv.text);

// ⑤ remove 删除
await runAction(stubHost, 'providers_remove', { id: np.id });
config.resetCache();
const listed4 = await runAction(stubHost, 'providers_list', {});
...
let terr1 = null;
try { await runAction(stubHost, 'transcribe', {}); } catch (e) { terr1 = e; }
assert(terr1 && /audio_path/.test(terr1.message), 'transcribe 缺 audio_path 报错', terr1 && terr1.message);
let terr2 = null;
try { await runAction(stubHost, 'transcribe', { audio_path: 'relative.wav' }); } catch (e) { terr2 = e; }
assert(terr2 && /绝对路径/.test(terr2.message), 'transcribe 相对路径报错', terr2 && terr2.message);
let terr3 = null;
try { await runAction(stubHost, 'transcribe', { audio_path: '/nonexistent/audio.wav' }); } catch (e) { terr3 = e; }
assert(terr3 && /不存在/.test(terr3.message), 'transcribe 文件不存在报错', terr3 && terr3.message);

// ⑧ 视频抽帧动作入参校验（阶段 7.1）
let verr1 = null;
try { await runAction(stubHost, 'video_frames', {}); } catch (e) { verr1 = e; }
assert(verr1 && /video_path/.test(verr1.message), 'video_frames 缺 video_path 报错', verr1 && verr1.message);
let verr2 = null;
try { await runAction(stubHost, 'video_frames', { video_path: 'relative.mp4' }); } catch (e) { verr2 = e; }
assert(verr2 && /绝对路径/.test(verr2.message), 'video_frames 相对路径报错', verr2 && verr2.message);
let verr3 = null;
try { await runAction(stubHost, 'video_frames', { video_path: '/nonexistent/video.mp4' }); } catch (e) { verr3 = e; }
...
// ⑨ 多模态摘要动作入参校验（阶段 7.3）
let merr1 = null;
try { await runAction(stubHost, 'media_summarize', {}); } catch (e) { merr1 = e; }
assert(merr1 && /video_path/.test(merr1.message), 'media_summarize 缺 video_path 报错', merr1 && merr1.message);
let merr2 = null;
try { await runAction(stubHost, 'media_summarize', { video_path: 'relative.mp4' }); } catch (e) { merr2 = e; }
assert(merr2 && /绝对路径/.test(merr2.message), 'media_summarize 相对路径报错', merr2 && merr2.message);
let merr3 = null;
try { await runAction(stubHost, 'media_summarize', { video_path: '/nonexistent/video.mp4' }); } catch (e) { merr3 = e; }
...
await runAction(stubHost, 'providers_set_models', { id: 'p_old', models: [
...
config.resetCache();
// ⑩a 设置有序 failover 列表
const poolBefore = (await runAction(stubHost, 'assignments_get', {})).poolByCapability.vision;
...
const setR = await runAction(stubHost, 'assignments_set', { capability: 'vision', model_refs: [strongRef, plusRef] });
assert(setR.ok && JSON.stringify(setR.model_refs) === JSON.stringify([strongRef, plusRef]), 'assignments_set 复合引用有序列表', setR);
// ⑩b get 返回归一化 order
const getR = await runAction(stubHost, 'assignments_get', {});
...
// ⑩e 非法分配（模型不具备该能力）→ 拒绝
let aerr = null;
try { await runAction(stubHost, 'assignments_set', { capability: 'vision', model_ids: ['wan2.2-t2i-flash'] }); } catch (e) { aerr = e; }
assert(aerr && /不具备该能力|不在全局池/.test(aerr.message), '给 vision 分配 image 模型被拒', aerr && aerr.message);
// ⑩f 空数组清除 → 回退池顺序
await runAction(stubHost, 'assignments_set', { capability: 'vision', model_ids: [] });
const getR2 = await runAction(stubHost, 'assignments_get', {});
...
// ⑩h 重复 id 自动去重
await runAction(stubHost, 'assignments_set', { capability: 'vision', model_ids: ['qwen-vl-plus', 'qwen-vl-plus', 'qwen3-vl-235b-a22b-thinking'] });
const getR3 = await runAction(stubHost, 'assignments_get', {});
...
const dupGet = await runAction(stubHost, 'assignments_get', {});
const cloneRef = dupGet.poolByCapability.vision.find((m) => m.providerId === clone.id && m.id === 'qwen-vl-plus').ref;
await runAction(stubHost, 'assignments_set', { capability: 'vision', model_refs: [cloneRef, plusRef] });
...
// 显式空索引模拟“作品库已经初始化”；自动接回旧 outputs 由 artifacts.mjs 单独覆盖。
const artifactsMod = await import('../lib/artifacts.js');
fs.writeFileSync(path.join(path.dirname(outDir), 'artifacts.json'), JSON.stringify({ version: 1, artifacts: [] }));
artifactsMod.resetCache();
// 造两条终态记录 + 一条 running + 各自产物文件 + 一个真正未入库的孤儿文件
...
const purge = await runAction(stubHost, 'tasks_purge_orphans', {});
assert(purge.ok && purge.deleted >= 1 && !fs.existsSync(path.join(outDir, 'orphan.png')), 'tasks_purge_orphans 删孤儿文件', purge.deleted);
// ⑪f 非法 scope 拒绝
let serr = null;
try { await runAction(stubHost, 'tasks_clear', { scope: 'bogus' }); } catch (e) { serr = e; }
...
for (const act of ['tasks_delete', 'tasks_clear', 'tasks_orphans', 'tasks_purge_orphans',
  'artifacts_reindex', 'artifacts_delete', 'artifacts_clear']) {
...
config.resetCache();
// ⑫a 手动添加模型（能力缺省按规则推断）
await runAction(stubHost, 'providers_add_model', { id: 'p_old', model_id: 'wan2.7-image' });
config.resetCache();
let plP3 = (await runAction(stubHost, 'providers_list', {})).providers.find((x) => x.id === 'p_old');
...
// ⑫b 纠正能力标签
await runAction(stubHost, 'providers_set_model_caps', { id: 'p_old', model_id: 'wan2.7-image', capabilities: ['vision'] });
config.resetCache();
plP3 = (await runAction(stubHost, 'providers_list', {})).providers.find((x) => x.id === 'p_old');
...
// ⑫e 移除模型
await runAction(stubHost, 'providers_remove_model', { id: 'p_old', model_id: 'wan2.7-image' });
config.resetCache();
plP3 = (await runAction(stubHost, 'providers_list', {})).providers.find((x) => x.id === 'p_old');
assert(!plP3.models.find((m) => m.id === 'wan2.7-image'), '⑫e 移除模型');
// ⑫f 空 model_id 拒绝
let aerr2 = null;
try { await runAction(stubHost, 'providers_add_model', { id: 'p_old', model_id: '' }); } catch (e) { aerr2 = e; }
...
const al = await runAction(stubHost, 'attachments_list', {});
...
const ex = await runAction(stubHost, 'attachment_export', { attachment_id: 'sha256:abc123' });
assert(ex.ok && ex.path && fs.existsSync(ex.path) && /gen\.png$/.test(ex.path), '⑬b attachment_export 返回真实路径', ex);
// ⑬c 未知附件（无 session/无上下文）→ 报错
let aer = null;
try { await runAction(stubHost, 'attachment_export', { attachment_id: 'sha256:zzz' }); } catch (e) { aer = e; }
assert(aer && /session_id|不可导出|不在本会话/.test(aer.message), '⑬c 未知附件导出报错', aer && aer.message);
// ⑬d 空 attachment_id 拒绝
let aer2 = null;
try { await runAction(stubHost, 'attachment_export', {}); } catch (e) { aer2 = e; }
