...
for (const act of ["action: 'image'", "action: 'video'", "action: 'tts'", "action: 'transcribe'", "action: 'video_frames'", "action: 'look'", "action: 'crop'", "action: 'diff'", "action: 'locate'", "action: 'html'", "action: 'ocr'", "action: 'relook'", "action: 'status'"]) {
  assert(src.includes(act), '缺少操作卡片 ' + act);
}

console.log('ALL OK —— 客户端形态 3 座位 + 3 组件 + 4 行为痕迹 + 阶段 5 操作卡片组 13 卡片断言全部通过');